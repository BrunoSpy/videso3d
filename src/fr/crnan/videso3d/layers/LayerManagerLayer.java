 ...
 */
public class LayerManagerLayer extends RenderableLayer implements SelectListener, MouseListener
{
protected VidesoGLCanvas wwd;
...
//Nombre de layers affichées dans la fenêtre
private int maxLayersOnScreen = 20;
private int UP = 1000;
private int DOWN = 2000;
private int displayIndex = 0;
private int oldDisplayIndex = 0;
private boolean upDownThreadRunning = false; 
...
protected Color dragColor = Color.RED;
    ...
    public LayerManagerLayer(VidesoGLCanvas wwd)
    {
        if (wwd == null)
        {
    ...
    protected void initialize()
    {
        ...
        this.wwd.addMouseListener(this);
    }
    ...
    public void selected(SelectEvent event)
    {
        if (event.hasObjects() && event.getTopObject() == this.annotation)
        {
            ...
            if (event.getEventAction().equals(SelectEvent.ROLLOVER)
                ...
                || event.getEventAction().equals(SelectEvent.LEFT_PRESS))
            {
                ...
                if(po.getValue(AVKey.URL) != null)
                {
                    ...
                    {
                    	if(i==UP || i==DOWN && !upDownThreadRunning){
                    		mousePressed = true;
                    		upDownThreadRunning = true;
                    		final int layersSize = layers.size();
                    		final int upOrDown = i;
                    		final int sleep = 50;
    ...
    protected String makeAnnotationText(LayerList layers)
    {
        ...
        String layerListText = new String();        
        int visibleLayerNames = 0;
        int numberOfLayers = maxLayersOnScreen-1;
        //S'il y a plus de 20 calques, on n'en affiche que 20 et on ajoute des flèches pour faire défiler la liste
        if(layers.size()>maxLayersOnScreen && !this.isMinimized()){
        	if(displayIndex==1){
        	...
        	if(displayIndex+maxLayersOnScreen<layers.size()-1)
        		text.append("<a href=\""+UP+"\"><font color=\""+encodeHTMLColor(Color.WHITE)+"\">.........\u21e7\u21e7\u21e7.........</a><br/>");
        	if(displayIndex+maxLayersOnScreen==layers.size()-1 || displayIndex==0){
        		numberOfLayers = maxLayersOnScreen; 
        	}
        	for(int j = layers.size()-numberOfLayers-displayIndex; j<=layers.size()-1-displayIndex; j++){
        		Layer layer = layers.get(j);
        		if (!this.isMinimized() && layer!=this)
        		{
        			text.append(makeLayerText(layer, j));
        			visibleLayerNames++;
        		}
        	}
        	if(displayIndex>0){
        		text.append("<a href=\""+DOWN+"\"><font color=\""+encodeHTMLColor(Color.WHITE)+"\">.........\u21e9\u21e9\u21e9.........</a><br/>");
        	}
        	//On ajoute le calque LayerList à la fin
        	text.append(makeLayerText(this, 0));
        	visibleLayerNames++;
        	text.append(layerListText.toString());
        }else{
        	int i = 0;
        	for (Layer layer : layers)
        	{
        		if (layer==this){
        			layerListText = makeLayerText(layer, i);
        			visibleLayerNames++;
        		}
        		else if (!this.isMinimized())
        		{
        			text.append(makeLayerText(layer, i));
        			visibleLayerNames++;
        		}
        ...
        if(visibleLayerNames>1){
            this.annotation.getAttributes().setSize(new Dimension(230,0));
        }else{
            this.annotation.getAttributes().setSize(new Dimension(100,0));
        }
        return text.toString();
    }

    private String makeLayerText(Layer layer, int layerIndex){
    	StringBuilder text = new StringBuilder();
    	color = (layerIndex == this.selectedIndex) ? this.highlightColor : this.color;
		color = (layerIndex == this.dragRefIndex) ? dragColor : color;
		text.append("<a href=\"");
		text.append(layerIndex);

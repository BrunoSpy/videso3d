...
public class AMSLGeographicTextRenderer extends GeographicTextRenderer {
    ...
    protected class OrderedText implements OrderedRenderable, Comparable<OrderedText>
    {
        ...
        private AMSLGeographicTextRenderer getRenderer()
        {
            ...
            try
            {
                if (cullText)
                {
                    ...
                    for (OrderedText ot : textList)
                    {
                        ...
                        Rectangle2D newBounds = AMSLGeographicTextRenderer.this.computeTextBounds(dc, ot,
                            scaleAndOpacity[0]);
                        if (newBounds == null)
                            continue;

                        boolean overlap = false;
                        newBounds = AMSLGeographicTextRenderer.this.computeExpandedBounds(newBounds, cullTextMargin);

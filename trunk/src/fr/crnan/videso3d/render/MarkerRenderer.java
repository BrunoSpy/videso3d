...
/**
 * Temporary class to fix vertical exaggeration
 * @author Bruno Spyckerelle
 *
 */
public class MarkerRenderer extends gov.nasa.worldwind.render.markers.MarkerRenderer{
	 ...
	 private double elevation = 10d;
	    ...
	    protected void drawAll(DrawContext dc, Iterable<Marker> markers)
	    {
	        this.begin(dc);
	        {
	            ...
	            if (dc.getFrameTimeStamp() != this.frameTimeStamp || dc.getVerticalExaggeration() != this.verticalExaggeration)
	            {
	    ...
	    protected Vec4 computeSurfacePoint(DrawContext dc, Position pos)
	    {
	        if (!this.overrideMarkerElevation)
	            return dc.getGlobe().computePointFromPosition(new Position(pos.latitude, pos.longitude, pos.elevation*dc.getVerticalExaggeration()));
